...
from buffer import *
buffer_test_ptn = 'buffer_(load|store|load_to_lds|load_to_lds_wrap)_ipw(\d+)_wpg(\d+)_dwpi(\d+)_so(\d+)_comp(\d+)_hit(\w+)'
...
INST_ARGS_MEMTYPE, INST_ARGS_IPW, INST_ARGS_WPG, INST_ARGS_DWPI, INST_ARGS_SOFFSET, INST_ARGS_COMP, INST_ARGS_HIT)
