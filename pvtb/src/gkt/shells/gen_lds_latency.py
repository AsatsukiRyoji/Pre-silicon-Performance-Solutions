...
from lds_latency import *
# 1. define test name pattern.
lds_test_ptn = 'perf_latency_lds_(\w+)_ipw(\d+)_wpg(\d+)'
...
inst_arg_str, all_args = LdsLtcClass.get_args_from_name(lds_test_ptn, INST_ARGS_INST, INST_ARGS_IPW, INST_ARGS_WPG)
...
svrtest = LdsLtcClass(inst_arg_str)
svrtest.gen_output()
