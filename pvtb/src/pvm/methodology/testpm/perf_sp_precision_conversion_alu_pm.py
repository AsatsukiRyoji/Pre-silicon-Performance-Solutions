...
dv_dir = cdir + '/../../shader/'
testpm_cfg_d = {
    'sp_precision_conversion_alu'   : {
        'dv'            : dv_dir + 'perf_sp_precision_conversion_alu.dv'
        ,'test_ptn'     : "perf_sp_v_\w+([0-9]+)_edc\w+_ipw(\d+)_wpg(\d+)" #need to update
        ,'test_param_l' : ['bitwise', 'valu', 'wpg']
        ,'warmup'       : 1
    }
}


class SPPrecisonConversionPM(PerfTestPM):
    
    def __init__(self):
        # call the __init__ func of parent class
        super(SPPrecisonConversionPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['sp_precision_conversion_alu'] 
        pass
