...
# -*- coding: utf-8 -*-
"""
Flat load (To VGPR) Performance Model
@author: Gui yiheng
"""
import os,sys,re
...
testpm_cfg_d = {
    'flat_load_stride'      : {
        'dv'            : dv_dir + 'perf_flat_load_stride.dv'
        ,'test_ptn'     : "perf_flat_([a-z]+)_dwordx(\d)_(hit[a-zA-Z]+)_ipw(\d+)_wpg(\d+)_tci(\d+)_inst0_bankhash(\d+)_stride(\d+)_isize(\d+)"
        ,'test_param_l' : ['vmemTYPE', 'vmemPAY', 'vmemCAC', 'vmem', 'wpg', 'tci']
        ,'warmup'       : 2
    }
}

class FlatStridePM(PerfTestPM):
    
    def __init__(self):
        super(FlatStridePM, self).__init__()
        self.test_cfg = testpm_cfg_d['flat_load_stride']
    
    def theory(self, desc, test = None, gui_en = False, **kw):
        """Flat Load (To VGPR) Theory
        ...
        #################
        ## sq setting####
        meta_df['salu']  = 3            # for global load dw, it shoud be 3 at less.
        meta_df['valu']  = 8            # for global load dw, it shoud be 8 at less.
        meta_df['smem']  = 1
        ...
        meta_df['vmemCOA'] = meta_df['name'].apply(lambda x:
               'uncoalesced' if((re.match("perf_flat_load_dwordx2_\w+", x))) else 'coalesced')
...
if __name__ == '__main__':
    ...
    flpm = FlatStridePM()
    if opt.func == 'theory':
        flpm.theory(opt.desc, opt.test, opt.gui_en)
    elif opt.func == 'measure':
        flpm.measure(opt.desc, opt.algo, opt.edir, opt.test, opt.gui_en)
    elif opt.func == 'check':
        flpm.check(opt.desc, opt.edir, opt.test, opt.gui_en)
    else:
        print('NYI')
        sys.exit()
