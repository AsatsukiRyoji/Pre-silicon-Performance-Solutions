...
testpm_cfg_d = {
    'gds'       : {
        'dv'            : dv_dir + 'perf_gds.dv'
        ,'test_ptn'     : "perf_gds_(\w+)_([a-z]+\d+)_ipw(\d+)_wpg(\d+)"
        ,'test_param_l' : ['itype', 'bitwise', 'ipw', 'wpg']
        ,'warmup'       : 1
    }
}


class GdsPM(PerfTestPM):
    
    def __init__(self):
        # call the __init__ func of parent class
        super(GdsPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['gds'] 
        pass
    ...
    def theory(self, desc, test = None, gui_en = False, **kw):
        ...
        meta_col = ['name',
                    ...
                    'itype', 'ipw', 'bitwise',                             # for sq theory 
                    ]
