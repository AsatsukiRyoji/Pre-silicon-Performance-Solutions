...
testpm_cfg_d = {
    'vgpr_ds_vgpr'        : {
        'dv'            : dv_dir + 'perf_vgpr_ds_vgpr.dv'
        ,'test_ptn'     : "perf_vgpr_ds_vgpr_(\w+)_ipw(\d+)_wpg(\d+)"
        ,'test_param_l' : ['lds_inst_type', 'ds_inst', 'wpg']
        ,'warmup'       : 1
    }
}

class VDsVPM(PerfTestPM):
    def __init__(self):
        # call the __init__ func of parent class
        super(VDsVPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['vgpr_ds_vgpr'] 
