...
dv_dir = cdir + '/../../command/'
testpm_cfg_d = {
    'cp_dispatch_multi_pipe_multi_queue'       : {
        'dv'            : dv_dir + 'perf_cp_dispatch.dv'
        ,'test_ptn'     : "perf_cp_dispatch_([a-zA-Z]+)pip_([a-zA-Z]+)que_dpq(\d+)_gpd(\d+)_wpg(\d+)"
        ,'test_param_l' : ['pip', 'que', 'dpq', 'gpd', 'wpg']
        ,'warmup'       : 0
    }
}

class CPSPIWaveLaunchPM(PerfTestPM):
    
    def __init__(self):
        # call the __init__ func of parent class
        super(CPSPIWaveLaunchPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['cp_dispatch_multi_pipe_multi_queue'] 
        pass
    ...
    def theory(self, desc, test = None, gui_en = False, **kw):
        ...
        meta_col = ['name',
                    'pip', 'que', 'dpq', 'gpd', 'wpg',      # for cp theory
        ...
        meta_df['que_send_delay'] = meta_df['pip'].replace(["two", "three", "all"], ["two_pipe", "three_pipe", "four_pipe"])
        meta_df['pip'] = meta_df['pip'].replace(["two", "three", "all"], [2,3,4])
        meta_df['que'] = meta_df['que'].replace(["single", "two", "four", "all"], [1,2,4,8])
