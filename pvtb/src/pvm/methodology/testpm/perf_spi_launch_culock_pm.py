...
dv_dir = cdir + '/../../command/'
testpm_cfg_d = {
    'spi_wave_launch_culock'       : {
        'dv'            : dv_dir + 'perf_spi_launch_culock.dv'
        ,'test_ptn'     : "perf_spi_launch_culock_ipw(\d+)_tg(\d+)_wpg(\d+)_pip(\d+)"
        ,'test_param_l' : ['valu', 'tg', 'wpg', 'pip']
        ,'warmup'       : 0
    }
}

class SPIWaveLaunchCulockPM(PerfTestPM):
    
    def __init__(self):
        # call the __init__ func of parent class
        super(SPIWaveLaunchCulockPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['spi_wave_launch_culock'] 
        pass
