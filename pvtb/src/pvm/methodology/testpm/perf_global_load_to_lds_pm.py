...
testpm_cfg_d = {
    'global_load_to_lds': {
        'dv'            : dv_dir + 'perf_global_load_to_lds.dv'
        ,'test_ptn'     : "perf_global_([a-z]+)_dwordx(\d)_toLDS_(hit[a-zA-Z]+)_ipw(\d+)_wpg(\d+)_inst(\d+)"
        ,'test_param_l' : ['vmemTYPE', 'vmemPAY', 'vmemCAC', 'vmem', 'wpg']
        ,'warmup'       : 1
    }
}

class GlobalLoadtoLdsPM(PerfTestPM):
    
    def __init__(self):
        super(GlobalLoadtoLdsPM, self).__init__()
        self.test_cfg = testpm_cfg_d['global_load_to_lds']
...
if __name__ == '__main__':
    ...
    gllpm = GlobalLoadtoLdsPM()
    if opt.func == 'theory':
        gllpm.theory(opt.desc, opt.test, opt.gui_en)
    elif opt.func == 'measure':
        gllpm.measure(opt.desc, opt.algo, opt.edir, opt.test, opt.gui_en)
    elif opt.func == 'check':
        gllpm.check(opt.desc, opt.edir, opt.test, opt.gui_en)
    else:
        print('NYI')
        sys.exit()
