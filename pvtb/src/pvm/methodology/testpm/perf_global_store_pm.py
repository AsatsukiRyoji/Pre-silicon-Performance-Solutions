...
# -*- coding: utf-8 -*-
"""
Global store Performance Model
@author: Ma Xingxing
"""
import os,sys,re
...
testpm_cfg_d = {
    'global_store'     : {
        'dv'            : dv_dir + 'perf_global_store.dv'
        ,'test_ptn'     : "perf_global_([a-z]+)_dwordx(\d)_(hit[a-zA-Z]+)_ipw(\d+)_wpg(\d+)_inst(\d+)"
        ,'test_param_l' : ['vmemTYPE', 'vmemPAY', 'vmemCAC', 'vmem', 'wpg']
        ,'warmup'       : 1
    }
}

class GlobalStorePM(PerfTestPM):
    
    def __init__(self):
        super(GlobalStorePM, self).__init__()
        self.test_cfg = testpm_cfg_d['global_store']

    def theory(self, desc, test = None, gui_en = False, **kw):
        """Global Load (To VGPR) Theory
        ...
        #################
        ## sq setting####
        meta_df['salu']  = 1            # for global store dw, it shoud be 3 at less.
        meta_df['valu']  = 10            # for global store dw, it shoud be 8 at less.
        meta_df['smem']  = 1
...
if __name__ == '__main__':
    ...
    gspm = GlobalStorePM()
    if opt.func == 'theory':
        gspm.theory(opt.desc, opt.test, opt.gui_en)
    elif opt.func == 'measure':
        gspm.measure(opt.desc, opt.algo, opt.edir, opt.test, opt.gui_en)
    elif opt.func == 'check':
        gspm.check(opt.desc, opt.edir, opt.test, opt.gui_en)
    else:
        print('NYI')
        sys.exit()
