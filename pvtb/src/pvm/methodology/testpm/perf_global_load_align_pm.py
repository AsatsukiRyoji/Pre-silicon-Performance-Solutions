...
# -*- coding: utf-8 -*-
"""
global load (To VGPR) 128 align Read Performance Model
@author: Xiong Jiayun
"""
...
testpm_cfg_d = {
    'global_load_align' : {
        'dv'            : dv_dir + 'perf_global_load_align.dv'
        ,'test_ptn'     : "perf_global_([a-z]+)_dwordx(\d)_hit([a-zA-Z]+)_ipw(\d+)_wpg(\d+)_align(\d+)"
        ,'test_param_l' : ['vmemTYPE', 'vmemPAY', 'vmemCAC', 'vmem', 'wpg','align']
        ,'warmup'       : 1
    }
}

class GlobalLoadAlignPM(PerfTestPM):
    def __init__(self):
        super(GlobalLoadAlignPM, self).__init__()
        self.test_cfg = testpm_cfg_d['global_load_align']

    def theory(self, desc, test = None, gui_en = False, **kw):
        """ 
        Global load (To VGPR) 128 Read ALIGN Performance Model
        :test: str(command line) or list or None(default, means every test in dv)
        """
        #print('[XIONG_DEBUG] theory:\n',desc, test, gui_en)
        ...
        meta_df['sgpr']  = 3            # S0 & S1 for get the global addr and S2 gets the threadgroup id
        ...
        #################
        ## sq setting####
        meta_df['salu']  = 1 #3            # for global load dw, it shoud be 3 at less.
        meta_df['valu']  = 10 #8            # for global load dw, it shoud be 8 at less.
        meta_df['smem']  = 1
        meta_df['soffset'] = 0          # for now, we just use global inst with Soffset
        ...
        #    'uncoalesced' if (re.match("perf_global_load_dwordx2_comp1_\w+", x)) else 'coalesced')
        ...
        #print('[XIONG_DEBUG] vmemCOA,coaRATIO,vmemPAY: \n',meta_df['vmemCOA'],meta_df['coaRATIO'],meta_df['vmemPAY'])
