...
testpm_cfg_d = {
    'flat_load'      : {
        'dv'            : dv_dir + 'perf_flat_load.dv'
        ,'test_ptn'     : "perf_flat_([a-z]+)_dwordx(\d)_(hit[a-zA-Z]+)_ipw(\d+)_wpg(\d+)_inst(\d+)"
        ,'test_param_l' : ['vmemTYPE', 'vmemPAY', 'vmemCAC', 'vmem', 'wpg']
        ,'warmup'       : 1
    }
}

class FlatLoadPM(PerfTestPM):
    
    def __init__(self):
        super(FlatLoadPM, self).__init__()
        self.test_cfg = testpm_cfg_d['flat_load']
    ...
    def theory(self, desc, test = None, gui_en = False, **kw):
        ...
        # Unit perf_flat_load_pm.pyis per wave
        ...
        #################
        ## sq setting####
        meta_df['salu']  = 2            # for global load dw, it shoud be 3 at less.
        meta_df['valu']  = 9            # for global load dw, it shoud be 8 at less.
        meta_df['smem']  = 1
