...
dv_dir = cdir + '/../../cache/'
testpm_cfg_d = {
    'buffer_load_to_lds_wrap' : {
        'dv'            : dv_dir + 'perf_buffer_load_to_lds_wrap.dv'
        ,'test_ptn'     : "perf_buffer_([a-z]+)_dwordx(\d)_toLDS_comp(\d)_hit([a-zA-Z]+)_ipw(\d+)_wpg(\d+)_wrap\d+"
        ,'test_param_l' : ['vmemTYPE', 'vmemPAY', 'component', 'vmemCAC', 'vmem', 'wpg']
        ,'warmup'       : 1
    }
}

class BufferLoadtoLdsWrapPM(PerfTestPM):

    def __init__(self):
        # call the __init__ func of parent class
        super(BufferLoadtoLdsWrapPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['buffer_load_to_lds_wrap']
    ...
    def theory(self, desc, test = None, gui_en = False, **kw):
        ...
        meta_df['vmemCOA'] = meta_df['name'].apply(lambda x: 
            'uncoalesced' if (re.match("perf_buffer_load_dwordx2_toLDS_comp2_\w+", x)) else 'coalesced')
        ## ta setting####
        #################
