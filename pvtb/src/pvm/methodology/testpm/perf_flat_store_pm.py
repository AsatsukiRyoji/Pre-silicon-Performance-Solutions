...
class FlatStorePM(PerfTestPM):
    
    def __init__(self):
        super(FlatStorePM, self).__init__()
        self.test_cfg = testpm_cfg_d['flat_store']
    ...
    def theory(self, desc, test = None, gui_en = False, **kw):
        ...
        meta_df['vmemCOA'] = meta_df['name'].apply(lambda x:
                'uncoalesced' if((re.match("perf_flat_store_dwordx2_\w+", x))) else 'coalesced')
        ## ta setting####
        #################
...
if __name__ == '__main__':
    ...
    fspm = FlatStorePM()
    if opt.func == 'theory':
        fspm.theory(opt.desc, opt.test, opt.gui_en)
    elif opt.func == 'measure':
        fspm.measure(opt.desc, opt.algo, opt.edir, opt.test, opt.gui_en)
    elif opt.func == 'check':
        fspm.check(opt.desc, opt.edir, opt.test, opt.gui_en)
    else:
        print('NYI')
        sys.exit()
