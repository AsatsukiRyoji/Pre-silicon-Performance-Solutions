...
testpm_cfg_d = {
    'matrix_copy_to_lds': {
        'dv'            : dv_dir + 'perf_matrix_copy_to_lds.dv'
        ,'test_ptn'     : "perf_matrix_copy_b(\d+)_sw([A-Z]+)_r[A-Z]+_t[A-Z]+_ipw(\d+)_lds"
        ,'test_param_l' : ['vmemBPE', 'vmemSW', 'vmem']
        ,'warmup'       : 1
    }
}

class MatrixCopytoLdsPM(PerfTestPM):

    def __init__(self):
        super(MatrixCopytoLdsPM, self).__init__()
        self.test_cfg = testpm_cfg_d['matrix_copy_to_lds']
