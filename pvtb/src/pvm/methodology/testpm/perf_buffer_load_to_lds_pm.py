...
testpm_cfg_d = {
    'buffer_load_to_lds' : {
        'dv'            : dv_dir + 'perf_buffer_load_to_lds.dv'
        ,'test_ptn'     : "perf_buffer_([a-z]+)_dwordx(\d)_toLDS_comp(\d)_(hit[a-zA-Z]+)_ipw(\d+)_wpg(\d+)_so\d_tid\d"
        ,'test_param_l' : ['vmemTYPE', 'vmemPAY', 'component', 'vmemCAC', 'vmem', 'wpg','soffset']
        ,'warmup'       : 1
    }
}

class BufferLoadtoLdsPM(PerfTestPM):

    def __init__(self):
        # call the __init__ func of parent class
        super(BufferLoadtoLdsPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['buffer_load_to_lds']
    ...
    def theory(self, desc, test = None, gui_en = False, **kw):
        ...
        meta_df['soffset'] = meta_df['name'].apply(lambda x: # for now, we can choose use soffset or not
                0 if (re.match("perf_buffer_([a-z]+)_dwordx(\d)_toLDS_comp(\d)_(hit[a-zA-Z]+)_ipw(\d+)_wpg(\d+)_so0_\w+", x)) else 1)
        ...
        meta_df['vmemCOA'] = meta_df['name'].apply(lambda x: 
            'uncoalesced' if (re.match("perf_buffer_load_dwordx2_toLDS_comp[02]_\w+", x)) else 'coalesced')
        ## ta setting####
        #################
