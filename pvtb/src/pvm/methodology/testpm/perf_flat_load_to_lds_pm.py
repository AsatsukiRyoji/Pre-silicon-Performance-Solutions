...
# -*- coding: utf-8 -*-
"""
Flat load (To LDS) Performance Model
@author: Gui yiheng
"""
import os,sys,re
...
testpm_cfg_d = {
    'flat_load_to_lds'  : {
        'dv'            : dv_dir + 'perf_flat_load_to_lds.dv'
        ,'test_ptn'     : "perf_flat_([a-z]+)_dwordx(\d)_toLDS_(hit[a-zA-Z]+)_ipw(\d+)_wpg(\d+)_inst(\d+)"
        ,'test_param_l' : ['vmemTYPE', 'vmemPAY', 'vmemCAC', 'vmem', 'wpg']
        ,'warmup'       : 1
    }
}

class FlatLoadtoLdsPM(PerfTestPM):
    
    def __init__(self):
        super(FlatLoadtoLdsPM, self).__init__()
        self.test_cfg = testpm_cfg_d['flat_load_to_lds']
    
    def theory(self, desc, test = None, gui_en = False, **kw):
        """Flat Load (To LDS) Theory
...
if __name__ == '__main__':
    ...
    fllpm = FlatLoadtoLdsPM()
    if opt.func == 'theory':
        fllpm.theory(opt.desc, opt.test, opt.gui_en)
    elif opt.func == 'measure':
        fllpm.measure(opt.desc, opt.algo, opt.edir, opt.test, opt.gui_en)
    elif opt.func == 'check':
        fllpm.check(opt.desc, opt.edir, opt.test, opt.gui_en)
    else:
        print('NYI')
        sys.exit()
