...
testpm_cfg_d = {
    'sp_trans_alu'   : {
        'dv'            : dv_dir + 'perf_sp_trans_alu.dv'
        ,'test_ptn'     : "perf_sp_v_\w+f([0-9]+)_edc\w+_ipw(\d+)_wpg(\d+)" #need to update
...
class SPTransPM(PerfTestPM):
    
    def __init__(self):
        # call the __init__ func of parent class
        super(SPTransPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['sp_trans_alu'] 
        pass
