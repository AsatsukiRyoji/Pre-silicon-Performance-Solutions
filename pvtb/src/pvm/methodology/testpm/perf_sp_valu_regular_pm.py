#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
SP Valu Performance Model
@author: Ma Xinging
"""
import os,sys,re
from pandas import DataFrame as DF
import pandas as pd
import pdb
import numpy as np

cdir = os.path.dirname(os.path.realpath(__file__)) + '/'
sys.path.append(cdir+'../')

import utility.util as util
from testpm.perf_test_pm import *
from theory.core_performance_theory import ComputeCoreTheory as CCT
from measure.core_performance_measure import ComputeCoreMeasure as CCM
from check.core_performance_check import ComputeCoreCheck as CCC

# test infomation about dv file, pattern, parameter and warm-up
dv_dir = cdir + '/../../shader/'
testpm_cfg_d = {
    'sp_valu_regular'   : {
        'dv'            : dv_dir + 'perf_sp_valu_regular.dv'
        ,'test_ptn'     : "perf_sp_valu_(\w+)_[a-z]+(\d+)_ipw(\d+)_wpg(\d+)" #need to update
        ,'test_param_l' : ['itype', 'bitwise', 'valu', 'wpg']
        ,'warmup'       : 1
    }
}


class SPValuRegularPM(PerfTestPM):
    
    def __init__(self):
        # call the __init__ func of parent class
        super(SPValuRegularPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['sp_valu_regular'] 
        pass
    
    def theory(self, desc, test = None, gui_en = False, **kw):
        '''SP VALU Theory
        :test: str(command line) or list or None(default, means every test in dv)
        '''

        super().theory(desc, test, gui_en, **kw)
        meta_col = ['name',
                    'wpg',                                                  # for cp theory
                    'vgpr', 'unorder',                                      # for spi theory
                    'valu', 'itype', 'bitwise',                             # for sq theory 
                    ]
        
        # [TODO]Passing missing labels to .loc will be deprecated, fix it at then
        # Unit is per wave
        meta_df = DF(self.test_info_df.loc[:,meta_col], columns = meta_col)
        
        ################
        ## cp setting###
        # get meta_df['wpg'] by testname with test_info_df
        ## cp setting###
        ################

        #################
        ## spi setting###
        meta_df['vgpr']  = 1            # thread_id in V0 is needed to control addr
        meta_df['unorder'] = 1          # do not use baton
        ## spi setting###
        #################
        
        #################
        ## sq setting####
        #meta_df['salu']     = 1            # for buffer load dw, it shoud be 3 at less.
        # get meta_df['valu'] by testname with test_info_df  = 8            
        #meta_df['smem']     = 0
        #meta_df['soffset']  = 0          # for now, we just use buffer inst with Soffset
        #meta_df['vmem'] = 0
        ## sq setting####
        #################

        #################
        ## sp setting####
        meta_df['vop32'] = meta_df['valu']      # for sp using, instead of sq issuing
        meta_df['bitwise'] = meta_df['bitwise'].astype(int)
        meta_df['bitwise'] = meta_df['bitwise'].astype(str)
        meta_df['vrate'] = "simd_exec_vop"
        meta_df['vrate'] = meta_df['vrate'].str.cat(meta_df['bitwise'])
        meta_df['total_flops'] = meta_df['itype']
        meta_df['itype'] = meta_df['itype'].replace(["fma", "add", "pk_fma", "pk_add", "rcp", "mov", "pk_mov", "pk_mul", "exp", "floor", "log", "swap", "mul", "ashrrev", "lshlrev", "mac", "max", "and_or", "mad", "sub", "min3", "or3", "fract", "not", "rndne", "xor", "min", "sad", "xad", "mul_hi", "mul_lo", "cmp_f", "cmp_eq", "cmp_gt", "cmp_t", "cmpx_f", "cmpx_ge", "cmpx_le", "cvt_f32", "cvt_sr_bf8", "cvt_sr_fp8", "cvt_bf16", "cvt_pk_fp8", "cvt_pk_bf8", "lshrrev"], ["normal", "normal", "pk", "pk", "trans", "normal", "normal", "normal", "trans", "normal", "trans", "half_rate", "normal", "normal", "normal", "normal", "normal", "normal", "half_rate", "normal", "half_rate", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal", "normal"])
        meta_df['itype'] = meta_df['name'].apply(lambda x: "half_rate" if(("fma_f16" in str(x)) or ("sub_i16" in str(x)) ) else meta_df['itype']) #when opcode is fma_f16 or sub_i16 and edc is on which need 8cycle to complete one inst(default edc is on)
        meta_df['itype'] = meta_df['name'].apply(lambda x: "normal" if(("pk_fma_f16" in str(x)) ) else meta_df['itype']) #when opcode is pk_fma_f16 and edc is on which need 8cycle to complete one inst(default edc is on)
        meta_df['itype'] = meta_df['name'].apply(lambda x: "normal" if("mad_f32" in str(x) ) else meta_df['itype'])#when opcode is mad_f32 and edc on need 4cycle(default edc is on)
        meta_df['itype'] = meta_df['name'].apply(lambda x: "normal" if(("mad_u64_u32" in str(x)) or ("mad_i64_i32" in str(x)) ) else meta_df['itype'])#when opcode is mad_i64_i32 and edc on need 4cycle(default edc is on)
        meta_df['vrate'] = meta_df['vrate'].str.cat(meta_df['itype'], sep="_")
        meta_df['freq'] = 1.36
        meta_df['total_flops'] = meta_df['total_flops'].replace(["fma", "add", "pk_fma", "pk_add", "rcp", "pk_mul", "exp", "log", "mul"], [2, 1, 2, 1, 4, 1, 4, 4, 1])

        # get meta_df['vmemPAY'] by testname with test_info_df
        # get meta_df['component'] by testname with test_info_df
        ## sp setting####
        #################
        
        ##################
        ## tcp setting####
        # get meta_df['vmemPAY'] by testname with test_info_df
        # get meta_df['vmemCAC'] by testname with test_info_df
        ## tcp setting####
        ##################

        ##################
        ## tcc setting####
        # get meta_df['vmemTYPE'] by testname with test_info_df
        ## tcc setting####
        ##################
        
        meta_cols = meta_df.columns #it will be changed by CCT
        cct = CCT(desc, meta_df)
        self.theo = cct.get_theory()
        
        if gui_en:
            gui = util.PMGui()
            gui.run([self.theo], [meta_cols])

        return self.theo

    def measure(self, desc, algo, edir = './', test = None, gui_en = False, **kw):
        """SPValuPM Measure
        :edir: str. It MUST be the parent dir contains sub-dir named as test name. 
        If user'd like to debug on some vecs, please also modify the name of dir to 
        comply with this. Else please use 'quickm' to invoke methods directly
        :test: str(command line) or list or None(default, means every test in dv)
        """

        # call the function in perf_test_pm
        super().measure(desc, algo, edir, test, gui_en, **kw)
    
    def check(self, desc, edir, test, gui_en = False, **kw):
        '''
        :test: str or list. 
        '''
        
        # call the function in perf_test_pm
        super().check(desc, edir, test, gui_en, **kw)

if __name__=='__main__':
    opt = util.option_parser()
    spvr = SPValuRegularPM()
    if opt.func == 'theory':
        spvr.theory(opt.desc, opt.test, opt.gui_en)
    elif opt.func == 'measure':
        spvr.measure(opt.desc, opt.algo, opt.edir, opt.test, opt.gui_en)
    elif opt.func == 'check':
        spvr.check(opt.desc, opt.edir, opt.test, opt.gui_en)
    else:
        print('NYI')
        sys.exit()


