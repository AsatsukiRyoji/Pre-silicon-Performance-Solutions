...
testpm_cfg_d = {
    'buffer_atomic'     : {
        'dv'            : dv_dir + 'perf_buffer_atomic.dv'
        ,'test_ptn'     : "perf_buffer_atomic_(\w+)_ipw(\d+)_wpg4"
        ,'test_param_l' : ['vmemPAY', 'vmem']
        ,'warmup'       : 1
    }
}

class BufferAtomicPM(PerfTestPM):
    def __init__(self):
        # call the __init__ func of parent class
        super(BufferAtomicPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['buffer_atomic'] 
