...
#cdir = '/project/bowen/a0/arch/liangyu/bowena0_pv_regr_0613/src/test/suites/block/perf/methodology/testpm/'
...
testpm_cfg_d = {
    'cp_dc_dispatch'       : {
        'dv'            : dv_dir + 'perf_cp_dc_dimx_dimy_dispatch.dv'
        ,'test_ptn'     : "perf_cp_dc_dimx_dimy_dispatch_dpq(\d+)_dimx(\d+)_dimy(\d+)_tpgx(\d+)_wpg(\d+)_mode(\d+)"
        ,'test_param_l' : ['dpq','dimx','dimy','tpgx','wpg','mode']
        ,'warmup'       : 0
    }
}

class CPDCXYDispatchPM(PerfTestPM):
    
    def __init__(self):
        # call the __init__ func of parent class
        super(CPDCXYDispatchPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['cp_dc_dispatch']
        pass
