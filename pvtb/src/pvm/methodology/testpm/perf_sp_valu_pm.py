...
testpm_cfg_d = {
    'sp_valu'       : {
        'dv'            : dv_dir + 'perf_sp_valu.dv'
        ,'test_ptn'     : "perf_sp_valu_(\w+)_\w(\d+)_ipw(\d+)_wpg(\d+)_(regular|gemm)"
        ,'test_param_l' : ['itype', 'bitwise', 'valu', 'wpg']
        ,'warmup'       : 1
    }
}

class SPValuPM(PerfTestPM):
    
    def __init__(self):
        # call the __init__ func of parent class
        super(SPValuPM, self).__init__()
    ...
    def theory(self, desc, test = None, gui_en = False, **kw):
        ...
        meta_df['itype'] = meta_df['itype'].replace(["fma", "add", "pk_fma", "pk_add", "rcp"], ["normal", "normal", "pk", "pk", "trans"])
