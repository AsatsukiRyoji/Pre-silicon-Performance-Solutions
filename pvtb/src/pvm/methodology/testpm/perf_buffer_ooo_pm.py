...
testpm_cfg_d = {
    'buffer_ooo' : {
        'dv'            : dv_dir + 'perf_buffer_ooo.dv'
        ,'test_ptn'     : "perf_buffer_\w+_instruction_mtype_\w+_ls_ooo_(ON|OFF)"
        ,'test_param_l' : ['oootype']
        ,'warmup'       : 1
    }
}

class BufferOooPM(PerfTestPM):
    def __init__(self):
        super(BufferOooPM, self).__init__()
        self.test_cfg = testpm_cfg_d['buffer_ooo']

    def theory(self, desc, test = None, gui_en = False, **kw):
        """Buffer Ooo (To VGPR) Theory
