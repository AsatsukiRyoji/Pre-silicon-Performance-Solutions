...
testpm_cfg_d = {
    'sp_valu'       : {
        'dv'            : dv_dir + 'perf_sp_valu_dpfp.dv'
        ,'test_ptn'     : "perf_sp_valu_(\w+)_[a-z]+(\d+)_(dpfp\d)_ipw(\d+)_wpg(\d+)"
        ,'test_param_l' : ['itype', 'bitwise', 'dpfp', 'valu', 'wpg']
        ,'warmup'       : 1
    }
}


class SPValuDPFPPM(PerfTestPM):
    
    def __init__(self):
        # call the __init__ func of parent class
        super(SPValuDPFPPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['sp_valu'] 
        pass
    ...
    def theory(self, desc, test = None, gui_en = False, **kw):
        ...
        meta_col = ['name',
                    ...
                    'valu', 'itype', 'bitwise', 'dpfp',                     # for sq theory
                    ]
        ...
        meta_df['vrate'] = meta_df['vrate'].str.cat(meta_df['dpfp'], sep="_")
        meta_df['freq'] = 1.36
        meta_df['total_flops'] = meta_df['itype'].apply(lambda x: 32 if x == "mmac_16x16x4" else 2)
        ## sp setting####
        #################
