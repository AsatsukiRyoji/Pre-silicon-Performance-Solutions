...
dv_dir = cdir + '/../../command/'
testpm_cfg_d = {
    'spi_wave_launch_initgpr'       : {
        'dv'            : dv_dir + 'perf_spi_launch_initgpr.dv'
        ,'test_ptn'     : "perf_spi_launch_initgpr_sgpr(\d+)_vgpr(\d+)_unorder([a-zA-Z]+)_ipw(\d+)_wpg(\d+)"
        ,'test_param_l' : ['sgpr', 'vgpr', 'unord', 'valu', 'wpg']
        ,'warmup'       : 1
    }
}

class SPIWaveLaunchInitGprPM(PerfTestPM):
    
    def __init__(self):
        # call the __init__ func of parent class
        super(SPIWaveLaunchInitGprPM, self).__init__()
        # get the key info from perf_test_pm
        self.test_cfg = testpm_cfg_d['spi_wave_launch_initgpr'] 
        pass
    ...
    def theory(self, desc, test = None, gui_en = False, **kw):
        ...
        #################
        ## spi setting###
        meta_df['unorder'] =  meta_df['unord'].apply(lambda x: 1 if (x == 'ON') else 0)
        ## spi setting###
        #################
