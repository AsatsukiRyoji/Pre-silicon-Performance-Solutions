...
testpm_cfg_d = {
    'buffer_load'       : {
        'dv'            : dv_dir + 'perf_buffer_load_stride.dv'
        ,'test_ptn'     : "perf_buffer_([a-z]+)_dwordx(\d)_comp(\d)_hit([a-zA-Z]+)_ipw(\d+)_wpg(\d+)_so(\d)_tci(\d+)_tid(\d)_bankhash(\d)_stride\d+(K|B)_isize(\d)"
        ,'test_param_l' : ['vmemTYPE', 'vmemPAY', 'component', 'vmemCAC', 'vmem', 'wpg', 'soffset', 'tci']
        ,'warmup'       : 1
    }
}

class BufferLoadStridePM(PerfTestPM):
    def __init__(self):
        # call the __init__ func of parent class
        super(BufferLoadStridePM, self).__init__()
    ...
    def theory(self, desc, test = None, gui_en = False, **kw):
        ...
        #    'coalesced' if (re.match("perf_buffer_load_dwordx4_\w+", x)) else 'uncoalesced')
    ...
    def measure(self, desc, algo, edir = './', test = None, gui_en = False, **kw):
        ...
        meta_df['warmup'] =  meta_df['name'].apply(lambda x:
                2 if (re.match("perf_buffer_([a-z]+)_dwordx(\d)_comp(\d)_hit([a-zA-Z]+)_ipw(\d+)_wpg(\d+)_so(\d)_tci(\d+)_tid(\d)_bankhash(\d)_stride(64K|8192K)\w*", x)) else 1)
