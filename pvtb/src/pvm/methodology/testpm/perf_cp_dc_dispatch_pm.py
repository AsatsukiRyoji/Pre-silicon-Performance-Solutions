...
testpm_cfg_d = {
    'cp_dc_dispatch'       : {
        'dv'            : dv_dir + 'perf_cp_dc_dispatch.dv'
        ,'test_ptn'     : "perf_cp_dc_dispatch_pipid(\d+)_queid(\d+)_dpq(\d+)_gpd(\d+)_wpg(\d+)"
        ,'test_param_l' : ['pip','que','dpq','gpd','wpg']
        ,'warmup'       : 0
    }
}

class CPDCDispatchPM(PerfTestPM):
    
    def __init__(self):
        # call the __init__ func of parent class
        super(CPDCDispatchPM, self).__init__()
